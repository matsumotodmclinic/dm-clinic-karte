...
import { normalizeFormData } from './normalizeFormData.js'
import {
  getCurrentMonth, buildAlcohol, buildSmoking, buildLiving, buildChildInfo,
  buildJobStr, dmOnsetText, buildWeekday, buildBmi, buildDmSymptoms, dropOther, dropOtherList,
} from './karteFields.js'
...
export function buildReasonFacts(d) {
  ...
  const ref = [r.referralFrom, dropOther(r.referralDept)].filter(Boolean).join(' ')
  ...
  if (r.thyroidConcern) {
    ...
    facts.push(['甲状腺疾患が気になる理由', [...arr.filter(x => x !== 'その他'), r.thyroidConcernNote].filter(Boolean).join('・') || '記載なし'])
  }
  // SAS 固有
  // ★JS 素組み側（buildReasonSummary）と同じく「その他」は落とす。
  //   ここだけ落とし忘れていたため、AI 経由のときだけ「受診理由：その他、…」と出ていた
  //   （2026-09-08 の 4 回目の検証で発覚。JS と AI で扱いが食い違う＝二重管理の小型版）
  if (r.purposes?.length) {
    const others = r.purposes.includes('その他') ? [r.purposeOther] : []
    facts.push(['受診理由', [...r.purposes.filter(x => x !== 'その他'), ...others].filter(Boolean).join('、')])
  }
  ...
  // ★自由記入欄（受診理由 step の「補足があれば記載」）は受診理由サマリーに入れない。
  //   受診理由ではない内容（症状・世間話）が書かれることが多く、サマリーが冗長になるため。
  //   代わりに申し送りの「□補足：…」に回す（院長判断 2026-09-08）→ freeNoteLine()
  return facts
}

// 自由記入欄 → 申し送りの1行。空なら行ごと出さない。
// ★AI には渡さず JS が決定論的に出す（生成のたびに載ったり落ちたりしないように）
export function freeNoteLine(d) {
  const t = (d?.reason?.summary || '').trim()
  return t ? `□補足：${t}` : ''
}

const joinNote = (arr, note) => {
  const a = (arr || []).join('、')
  if (!a) return (note || '').trim()          // 選択肢が空なら補足をそのまま出す（括弧を浮かせない）
...
export function buildReasonSummary(d) {
  ...
  const parts = []
  if (r.type === '紹介') {
    const from = [r.referralFrom, dropOther(r.referralDept)].filter(Boolean).join(' ')
...
// 同じ疾患が構造化データと音声の両方にあると 2 行になる。
// 統合は本来 AI の仕事だが、**AI が落ちたときに重複したカルテが保存されないよう**
// 素組み側でも病名が同じ行は 1 行にまとめる（情報量の多い方を残す）。
export function dedupePastHistory(lines) {
  const byName = new Map()
  for (const l of lines) {
    const name = String(l).replace(/^[♯＃]/, '').split(/[（(]/)[0].trim()
    const cur = byName.get(name)
    if (!cur || String(l).length > cur.length) byName.set(name, l)
  }
...
function pastHistoryLines(d, merged) {
  return Array.isArray(merged?.pastHistory) && merged.pastHistory.length
    ...
    : dedupePastHistory(buildPastHistoryLines(d))
}
...
function buildBodyLine(d, { pregnancy = false, short = false } = {}) {
  ...
  // 「0」は打ち間違い。そのまま出すと「身長:0cm」になるので未入力扱いにする。
  // 単位付き（DB 直編集で「60kg」等が入りうる）は数値部分だけ取り出す（「60kgkg」を防ぐ）
  const num = v => (parseFloat(v) > 0 ? String(parseFloat(v)) : '○')
  const head = `身長:${num(b.height)}cm　初診時:${num(b.weightNow)}kg${bmi ? `（BMI ${bmi}）` : ''}`
  if (short) return head
  const pre = pregnancy ? `　妊娠前:${num(b.weightPregnancy)}kg` : ''
  return `${head}${pre}　20歳時:${num(b.weight20)}kg　max体重${num(b.weightMax)}kg(${num(b.weightMaxAge)}歳)`
}
...
// ── 申し送り: 全フォーム共通の先頭 ─────────────────────────
function handoffHead(d, mergeFailed) {
  const lines = ['□通院のご案内をお渡し済']
  const note = freeNoteLine(d)
  if (note) lines.push(note)
  // ★AI 統合に失敗したまま黙って保存されると、質が落ちたことに誰も気付けない
  if (mergeFailed) lines.push('□AI統合に失敗：受診理由サマリーと♯既往歴を確認してください')
...
// DM基本
// ══════════════════════════════════════════════════════════
function buildDM(d, now, merged, mergeFailed) {
  ...
  out.push(...handoffHead(d, mergeFailed))
...
// 1型糖尿病
// ══════════════════════════════════════════════════════════
function buildT1D(d, now, merged, mergeFailed) {
  const month = getCurrentMonth(now)
  const dis = d?.disease || {}
  const h = d?.history || {}
  const r = d?.reason || {}
  ...
  // ★未聴取（空欄）を「受給困難」と断定しない（院長判断 2026-09-06 の「未入力と『なし』を書き分ける」）
  const kosei = dis.pensionKosei === 'はい（加入していた）' ? '有'
    : dis.pensionKosei === 'いいえ（未加入）' ? '無' : '未聴取'
  const pension = dis.pensionStatus === '受給中' ? '受給中'
    : dis.pensionKosei === 'はい（加入していた）' ? 'CPR次第'
      : dis.pensionKosei === 'いいえ（未加入）' ? '受給困難（×）' : '判定不能（要確認）'
  ...
  const device = (cur, w) => (using(cur) ? (cur === w ? `${w}（継続）` : `${cur}使用中→${w}`) : w)
  if (wish(r.cgmWish)) out.push(`□CGM：${device(r.cgmCurrent, r.cgmWish)}`)
  if (wish(r.pumpWish)) out.push(`□インスリンポンプ：${device(r.pumpCurrent, r.pumpWish)}`)
  out.push(...buildStaffFlagLines(d?.body))
  out.push(`【診察にあたっての要望】${d?.body?.concern || 'なし'}`)
  out.push(DIVIDER)
...
// 小児1型糖尿病
// ══════════════════════════════════════════════════════════
function buildPedT1D(d, now, merged, mergeFailed) {
  const month = getCurrentMonth(now)
  const dis = d?.disease || {}
  const sup = d?.support || {}
  const ch = d?.chronic || {}
  ...
  out.push(`・居住地：${dropOther(ch.residenceCity)}`)
  if (dis.ht) out.push('＃HT')
  if (dis.hl) out.push('＃HL')
  ...
  out.push('【協力体制】')
  const familySub = dropOtherList(sup.familySubList).join('・')
  const familyMain = dropOtherList(sup.familyMain)
  out.push(`①家族の協力体制：${[familyMain ? `主な管理者は${familyMain}` : '', familySub ? `サポート：${familySub}` : '', sup.familyNote].filter(Boolean).join('、')}`)
  out.push(`②学校の協力体制：${dropOtherList(sup.schoolStaff).join('・')}`)
  out.push(`③学校でサポートしてくれる人：${[dropOtherList(sup.schoolSupportPerson).join('・'), sup.schoolSupportNote].filter(Boolean).join('、')}`)
  out.push(`④開示状況（クラスメート）：${sup.disclosedChild || ''}`)
  out.push(`④開示状況（先生）：${sup.disclosedTeacher || ''}`)
  out.push(`【本人のスケジュール】${[sup.childGrade, dropOtherList(sup.childActivities).join('・'), sup.childActivityNote].filter(Boolean).join('、')}`)
  out.push(`【親のスケジュール】${[
    dropOtherList(sup.parentWorkMain).length || sup.parentWorkMainNote ? `母：${[dropOtherList(sup.parentWorkMain).join('・'), sup.parentWorkMainNote].filter(Boolean).join('（') + (dropOtherList(sup.parentWorkMain).length && sup.parentWorkMainNote ? '）' : '')}` : '',
    dropOtherList(sup.parentWorkSub).length || sup.parentWorkSubNote ? `父：${[dropOtherList(sup.parentWorkSub).join('・'), sup.parentWorkSubNote].filter(Boolean).join('（') + (dropOtherList(sup.parentWorkSub).length && sup.parentWorkSubNote ? '）' : '')}` : '',
  ].filter(Boolean).join('　')}`)
...
// ══════════════════════════════════════════════════════════
function buildHthlOrEndocrine(d, now, merged, mergeFailed, { endocrine }) {
  const month = getCurrentMonth(now)
  const dis = d?.disease || {}
...
// 妊娠糖尿病
// ══════════════════════════════════════════════════════════
function buildGDM(d, now, merged, mergeFailed) {
  const month = getCurrentMonth(now)
  const dis = d?.disease || {}
  const h = d?.history || {}
  const out = []
  ...
  // ★病名未選択のときは断定しない（DM基本の「＃糖尿病 or IGT or 正常耐糖能」と同じ扱い）
  out.push(!dis.dmType ? '＃妊娠糖尿病 or 糖尿病合併妊娠'
    : isCombined ? '＃糖尿病合併妊娠' : '＃妊娠糖尿病')
  const due = dis.dueDateYear ? `${dis.dueDateEra || '令和'}${dis.dueDateYear}年${dis.dueDateMonth || ''}月` : ''
  out.push(`　現在${dis.currentWeek || ''}週${due ? `、${due}` : ''}`)
  const obName = dis.obHospital === 'その他'
    ? (dis.obHospitalOther || '')
    : (dis.obHospital || (dis.obHospitalOther || '').trim())
  out.push(`　産科通院先：${obName}`)
  const pastGdm = (() => {
...
// 反応性低血糖
// ══════════════════════════════════════════════════════════
function buildRH(d, now, merged, mergeFailed) {
  const month = getCurrentMonth(now)
  const s = d?.symptom || {}
  const hasDm = hasDmDiff(d)
  const out = []
  ...
  if (freeNoteLine(d)) out.push(freeNoteLine(d))
  if (mergeFailed) out.push('□AI統合に失敗：受診理由サマリーと♯既往歴を確認してください')
...
function buildSAS(d, now, merged, mergeFailed) {
  const month = getCurrentMonth(now)
  const r = d?.reason || {}
  const dis = d?.disease || {}
  const hasDm = hasDmDiff(d)
...
function buildThyroid(form_type, d, now, merged) {
  ...
  const diagnosisName = (() => {
    ...
    if (t === 'basedow-cont') {
      return h.diagnosisYear
        ? `＃バセドウ病　甲状腺機能亢進症（診断時期：${h.diagnosisEra || '令和'}${h.diagnosisYear}年）`
        : '＃バセドウ病　甲状腺機能亢進症（診断時期不明）'
    }
  ...
  // buildDmSymptoms と同じ扱い（「その他」を畳んで「その他: ○○」にする）
  const symptoms = buildDmSymptoms(d?.symptom)
  const reasonText = (() => {
    const r = d?.reason || {}
    const parts = []
    if (r.thyroidConcern) {
  ...
  // 自由記入があれば AI が文章化した結果を使う。自覚症状の一文は JS 側で必ず付ける
  // （「○○の訴えあり。」という定型を AI に落とされないため）
  const reasonPart = (merged?.reasonSummary || '').trim() || reasonText
  const summary = [
    reasonPart ? (reasonPart.endsWith('。') ? reasonPart : `${reasonPart}。`) : '',
...
const BUILDERS = {
  ...
  '1型糖尿病': buildT1D,
  '小児1型糖尿病': buildPedT1D,
  '高血圧・脂質異常症': (d, now, merged, failed) => buildHthlOrEndocrine(d, now, merged, failed, { endocrine: false }),
  '内分泌': (d, now, merged, failed) => buildHthlOrEndocrine(d, now, merged, failed, { endocrine: true }),
  '妊娠糖尿病': buildGDM,
  '反応性低血糖': buildRH,
  '睡眠時無呼吸症候群': buildSAS,
}
for (const label of Object.keys(THYROID_TYPE)) {
  BUILDERS[label] = (d, now, merged) => buildThyroid(label, d, now, merged)
}
...
// mergeFailed = 統合が必要だったのに AI から結果を得られなかった
//               （通信エラー・壊れた返答）。申し送りに 1 行足して気付けるようにする
export function buildKarteTemplate(form_type, form_data, { now, merged, mergeFailed = false } = {}) {
  const build = BUILDERS[form_type]
  if (!build) return null
  // ★型を入口で揃える。配列であるはずの項目に文字列が入っていると
  //   組立の途中で例外になり「カルテが作れない」状態になるため（2026-09-08）
  return build(normalizeFormData(form_data), now, merged, mergeFailed)
}

// ── AI に頼む「統合」だけのプロンプト ──────────────────────
//
// AI がやるのは ①話し言葉の文章化と構造化データとの統合 ②♯既往のマージ・重複排除 の 2 つ。
//
// ★AI を呼ぶ条件 = **文章化が要る自由記述があるとき**。
//   ①音声入力（入力時に voiceSummary.js が整形済み。それを他の材料と統合する）
//   ②キーボードで打った自由記述（受診理由の自由記入欄・「気になる理由」の詳細など）
//      ← 2026-09-07 追加。素通しだと話し言葉のまま・「3年前」が和暦に直らずカルテに載る
//
//   選択式のボタン（紹介の経緯・転院理由・気になる理由の選択肢）は文言が固定なので
//   そのまま連結してよい = AI は要らない。
//
// これらが 1 つも無ければ null を返し、AI を呼ばない（全部選択式で完結した回）。
// ★reason.summary（自由記入欄）はここに入れない。
//   受診理由サマリーに混ぜず、申し送りの「□補足：」に逐語で出すようにしたため
//   （院長判断 2026-09-08）、AI に渡す必要が無くなった。
const FREE_TEXT_PATHS = [
  ['reason', 'dmConcernNote'],      // DM「糖尿病が気になる理由」の詳細
  ['reason', 'thyroidConcernNote'], // 甲状腺「気になる理由」の その他
  ['reason', 'purposeOther'],       // SAS 受診理由の その他
]

export function needsMerge(form_data) {
  const d = form_data || {}
  if ((d.voiceMemo?.aiSummary || '').trim() || (d.voicePastHistory?.aiSummary || '').trim()) return true
  return FREE_TEXT_PATHS.some(([a, b]) => String(d?.[a]?.[b] || '').trim())
}

export function buildMergePrompt(form_type, form_data, { now } = {}) {
  if (!BUILDERS[form_type]) return null
  if (!needsMerge(form_data)) return null

  const d = normalizeFormData(form_data)
...
【共通ルール】
- 本日は ${getCurrentMonth(now)}（和暦）
- 時期は和暦のみ（H8 / R5 等）。西暦・年齢は使わない
- ★「去年」「昨年」「一昨年」「半年前」「3年前」のような相対表現は、本日を基準に和暦へ換算する
  （例: 本日が R8 なら「去年」→ R7、「3年前」→ R5）。換算できないときは時期を書かない
- 与えられていない情報を推測して足さない
- 「不明」などの穴埋めをしない

【1. 受診理由サマリー】
下の材料を文にまとめてください。材料が何も無ければ空文字にしてください。
- ★**下の材料はどれ 1 つ省略しない**
- 同じ内容が複数の材料にあるときだけ 1 回にまとめる
- 簡潔に書く（目安 2〜3 行）。材料が多くて収まらないときは、削るのではなく行を足す
...
// AI が括弧を半角にして返す回があるので、カルテの書式（全角）に揃える。
// ★プロンプトで頼むだけにせず JS で直す（決定論的に直せるものは AI に任せない）
//   2026-09-08 の実測: 3 回中 1 回「♯慢性腎臓病(上尾中央総合病院腎臓内科…)」と半角になった
const toFullWidthParens = t => String(t).replace(/\(/g, '（').replace(/\)/g, '）')
...
export function parseMergeResponse(text) {
  ...
  try {
    ...
    return {
      reasonSummary: typeof o.reasonSummary === 'string' ? toFullWidthParens(o.reasonSummary) : '',
      pastHistory: Array.isArray(o.pastHistory)
        ? o.pastHistory.filter(x => typeof x === 'string').map(toFullWidthParens)
        : [],
    }
  } catch {
    return null
