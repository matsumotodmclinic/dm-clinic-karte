...
import {
  ...
  buildJobStr, dmOnsetText, buildWeekday, buildBmi, buildDmSymptoms, dropOther,
} from './karteFields.js'
...
function buildPedT1D(d, now, merged, mergeFailed) {
  ...
  out.push('【協力体制】')
  const familySub = dropOther(sup.familySubList).join('・')
  const familyMain = dropOther(sup.familyMain)
  out.push(`①家族の協力体制：${[familyMain ? `主な管理者は${familyMain}` : '', familySub ? `サポート：${familySub}` : '', sup.familyNote].filter(Boolean).join('、')}`)
  out.push(`②学校の協力体制：${dropOther(sup.schoolStaff).join('・')}`)
  out.push(`③学校でサポートしてくれる人：${[dropOther(sup.schoolSupportPerson).join('・'), sup.schoolSupportNote].filter(Boolean).join('、')}`)
  out.push(`④開示状況（クラスメート）：${sup.disclosedChild || ''}`)
  out.push(`④開示状況（先生）：${sup.disclosedTeacher || ''}`)
  out.push(`【本人のスケジュール】${[sup.childGrade, dropOther(sup.childActivities).join('・'), sup.childActivityNote].filter(Boolean).join('、')}`)
  out.push(`【親のスケジュール】${[
    dropOther(sup.parentWorkMain).length || sup.parentWorkMainNote ? `母：${[dropOther(sup.parentWorkMain).join('・'), sup.parentWorkMainNote].filter(Boolean).join('（') + (dropOther(sup.parentWorkMain).length && sup.parentWorkMainNote ? '）' : '')}` : '',
    dropOther(sup.parentWorkSub).length || sup.parentWorkSubNote ? `父：${[dropOther(sup.parentWorkSub).join('・'), sup.parentWorkSubNote].filter(Boolean).join('（') + (dropOther(sup.parentWorkSub).length && sup.parentWorkSubNote ? '）' : '')}` : '',
  ].filter(Boolean).join('　')}`)
...
export function buildKarteTemplate(form_type, form_data, { now, merged, mergeFailed = false } = {}) {
  ...
  return build(form_data || {}, now, merged, mergeFailed)
}
