...
export async function generateKarteText(form_type, form_data, callAI) {
  ...
  // ★統合が必要だったのに得られなかったときは申し送りに 1 行足す。
  //   素組みは同じ疾患が構造化と音声で 2 行になりうるので、黙って保存されると
  //   質が落ちたことに誰も気付けない（2026-09-08 の 2 回目の検証で判明）。
  const mergeFailed = !!prompt && !merged
  return buildKarteTemplate(form_type, form_data, { merged, mergeFailed })
}
