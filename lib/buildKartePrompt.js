// カルテ生成プロンプトの組み立て（経路B: 詳細画面の再生成）
//
// 2026-08-22 に pages/api/generate-karte.js から切り出した。理由:
//   ・API ハンドラの中にあるとテストから呼べず、プロンプト内容を検証できなかった
//     (2026-08-22 の hospitalOther 取りこぼしは型でもビルドでも落ちない種類のバグ)
//   ・scripts/check-prompt-sync.mjs / scripts/golden/ から直接 import して検証する
// ハンドラ側は 入力検証 → buildKartePrompt() → Anthropic 呼び出し だけを持つ。

import { buildOtherDiseasesText, pickOtherDiseases } from './otherDiseases.js'
import { formatEcho, buildEchoLine } from './echo.js'
import { hasDmDiff, buildDmDiffBlock, buildDmDiffRules } from './dmDiff.js'
import { buildStaffFlagLines, buildStaffFlagsBlock } from './handoffNotes.js'
import {
...
export class UnsupportedFormTypeError extends Error {
  constructor(formType) {
    super(`未対応のform_type: ${formType}`)
    this.name = 'UnsupportedFormTypeError'
    this.formType = formType
  }
}

// form_type と form_data から Anthropic に投げるプロンプトを組み立てる。
// 戻り値: { prompt, max_tokens }
// 未対応の form_type は UnsupportedFormTypeError を throw する。
export function buildKartePrompt(form_type, form_data) {
  const d = form_data

  // ── 項目の文字列組立は lib/karteFields.js に集約（AI フリー版と共通）──────
  // 呼び出し側の記述を変えずに済むよう、d を束ねた薄いラッパにしておく
  const alcoholText   = () => buildAlcohol(d)
  const smokingText   = () => buildSmoking(d)
  const livingText    = () => buildLiving(d)
  const childInfoText = () => buildChildInfo(d)
  const jobText       = () => buildJobStr(d)
  const onsetText     = () => dmOnsetText(d)
  const weekdayText   = () => buildWeekday(d)
  ...
  const bmiNow = buildBmi(d)
  const bmiSuffix = bmiNow ? `（BMI ${bmiNow}）` : ''

  // 糖尿病の症状（DM基本 / 1型 / 小児1型）
  const dmSymptomsText = buildDmSymptoms(d.disease?.dmSymptoms)
  const dmSymptomsBlock = dmSymptomsText
    ? `\n【糖尿病の症状(チェック済、横一列に「・」区切りで【糖尿病の症状】セクションに記載)】\n${dmSymptomsText}\n`
    : ''

  // スタッフ入力由来の申し送り行（新患2枠・医師希望・患者フラグ）は JS 側で決定論的に組み立てる
  const staffFlagsBlock = buildStaffFlagsBlock(d.body)
...
${voiceMemoBlock}${voicePastHistoryBlock}${voiceMemoNeedsReviewBlock}${needsDoctorReviewBlock}${dmSymptomsBlock}
【出力フォーマット（必ずこの順序で。該当なければ省略）】
（体重減少が「あり」かつ3kg以上の場合のみ）【⚠️ 体重減少あり・早急なインスリン導入を検討】

${getCurrentMonth()}：（受診理由サマリー1〜2行。記載なければ省略${voiceMemoNote}）
${d.reason?.dmConcern ? '＃糖尿病 or IGT or 正常耐糖能' : `＃糖尿病${onsetText()}`}（サマリーの直後、空行なし）
＃HT（該当時のみ）
＃HL（該当時のみ）
...
${staffFlagsBlock}【診察にあたっての要望】（記載あれば内容を、なければ「なし」と記載）
---------------------------------------------
${COMMON_FOOTER}
DM基本セット
1月follow
...
- 該当しない項目は省略する
- フォーマット記号（＃【】□♯）を使用する
- 空行ルール（厳守）: ①自院管理＃疾患（＃1型糖尿病/＃HT/＃HL等）は連続列挙し空行なし ②自院管理ブロックの後、他院管理疾患の前にのみ1行空ける ③他院管理疾患が複数あっても他院管理同士は連続列挙し空行なし ④他院管理疾患の最終行と【アレルギー歴】の間は空行なし（直接続ける） ⑤【事前聴取時 申し送り事項】の最終□行と【診察にあたっての要望】の間も空行なし（連続）
- 体重減少ありの場合は一番上に【⚠️ 体重減少あり・早急なインスリン導入を検討】と記載
...
＃1型糖尿病（${d.disease?.dm1type || 'タイプ不明'}）${onsetText()}
・GAD抗体：（初診時採血）
・CPR：（初診時採血）
...
（デバイス希望がある場合）□使用希望デバイス：CGM=${d.reason?.cgmWish || 'なし'}　ポンプ=${d.reason?.pumpWish || 'なし'}
...
`
  ...
  // ────── 高血圧・脂質異常症 ──────
    ...
    // dmDiff: 事前採血の HbA1c 高値で糖尿病が判明した場合、
    // 詳細画面でスタッフが追加聴取した内容（lib/dmDiff.js）
    const dm = d.dmDiff || {}
    const hasDm = hasDmDiff(d)
    const dmDiffBlock = buildDmDiffBlock(d.dmDiff)
...
- 該当しない項目は省略する
- フォーマット記号（＃【】□♯）を使用する
- 空行ルール（厳守）: ①自院管理＃疾患（＃HT/＃HL等）は連続列挙し空行なし ②自院管理ブロックの後、他院管理疾患の前にのみ1行空ける ③他院管理疾患が複数あっても他院管理同士は連続列挙し空行なし ④他院管理疾患の最終行と【アレルギー歴】の間は空行なし（直接続ける） ⑤【事前聴取時 申し送り事項】の最終□行と【診察にあたっての要望】の間も空行なし（連続）
- 60歳未満はワクチン歴を省略、70歳未満は子供の状況を省略
- 喫煙歴は「○本×○年（○歳〜）」の形式
${hasDm ? buildDmDiffRules('＃糖尿病（採血で判明）を受診理由サマリーの直後（＃HT/＃HLよりも前）に空行なしで記載し、必要なら発症時期も記載。IGT が選択されていても今回の採血で糖尿病が判明しているため、＃IGT は記載せず ＃糖尿病 とする') + '\n' : ''}
【整形済みデータ】
飲酒歴：${alcoholText()}
喫煙歴：${smokingText()}
生活情報：${livingText()}
...
【患者情報JSON】
${JSON.stringify(d, null, 2)}
${voiceMemoBlock}${voicePastHistoryBlock}${voiceMemoNeedsReviewBlock}${needsDoctorReviewBlock}${dmDiffBlock}
【ルール追加】
...
【出力フォーマット】
${getCurrentMonth()}：（受診理由1〜2行。「気になって受診」の場合は気になる理由も含めて記載。自由記入欄の内容も含める${voiceMemoNote}${hasDm ? '。採血でDM判明したことも触れる' : ''}）
${hasDm ? '＃糖尿病（採血で判明、発症時期はDM差分問診に従う）（受診理由の直後、空行なし）\n' : '＃IGT（該当時のみ、受診理由の直後、空行なし）\n'}＃HT（該当時のみ、空行なし）
＃HL（該当時のみ、空行なし）
...
【飲酒歴】（整形済みテキスト）
【喫煙歴】（整形済みテキスト）
${hasDm ? '【眼科通院歴】（DM差分問診の眼底検査・網膜症から記載。受けていない場合は「未受診」）\n' : ''}【健診】
【ワクチン歴】（60歳以上のみ）
【生活情報】（整形済みテキスト。70歳以上は子供の状況も含む）
【仕事】職業・活動量
---------------------------------------------
...
身長:○cm　初診時:○kg${bmiSuffix}　20歳時:○kg　max体重○kg(○歳)
---------------------------------------------
${hasDm && dm.dmSymptoms?.selected?.length ? '【糖尿病の症状】（チェックされた症状を「・」で横一列に記載。「その他」がチェックされていれば末尾に「その他: ○○」を追加）\n---------------------------------------------\n' : ''}【事前聴取時　申し送り事項】
□通院のご案内をお渡し済
（現病歴：要DR確認フラグありの場合のみ）□現病歴：問診時間の関係で一部省略、要DR確認
（既往歴：要ドクター確認フラグありの場合のみ）□既往歴：要ドクター確認
${hasDm ? '□採血で DM判明 → DM初期評価追加実施済\n' : ''}${hasDm && (dm.eyeFundusCheck === '受けていない' || dm.eyeNotebook === '持っていない') ? '□糖尿病-眼科連携手帳をお渡し\n' : ''}${hasDm && dm.weightLoss === 'あり（3kg以上）' ? '□体重減少あり（3ヶ月以内に3kg以上）インスリン導入要検討\n' : ''}（HLありの場合）□健診・前医採血でLDL-C140mg/dl以上のため、甲状腺3項目を追加しました。
□初回療養計画書を作成済
${staffFlagsBlock}（その他該当する申し送り事項があれば記載）
【診察にあたっての要望】（記載あれば内容を、なければ「なし」と記載）
---------------------------------------------
${hasDm ? COMMON_FOOTER : `${getCurrentMonth()}：
...
（アレルギー薬がある場合のみ「⚠️○○アレルギー⚠️」と1行で記載。HTMLタグ・style属性は絶対に出力しない。プレーンテキストのみ）
目標HbA1c　　　　%　目標体重　　　次回検討薬：`}
${hasDm ? 'DM基本セット' : '基本採血なし'}
1月follow
${weekdayText()}
LINE登録ご案内→済　登録確認未・登録できない
`
    max_tokens = hasDm ? 2000 : 1500
...
- 60歳未満はワクチン歴を省略、70歳未満は子供の状況を省略
- 喫煙歴は「○本×○年（○歳〜）」の形式
${hasDm ? buildDmDiffRules('＃糖尿病（採血で判明）を受診理由の直後に空行なしで記載する。これは上の「＃で始まる行は出力しない」ルールの唯一の例外（採血で確定しており推測ではないため）。内分泌の主病名は引き続き出力せず、申し送りの「□主病名：医師の診察時に確定・記載」もそのまま残す') + '\n' : ''}
【整形済みデータ】
...
【出力フォーマット】
${getCurrentMonth()}：（受診理由1〜2行。「気になって受診」の場合は気になる理由も含めて記載。自由記入欄の内容も含める${voiceMemoNote}${hasDm ? '。採血でDM判明したことも触れる' : ''}）
${hasDm ? '＃糖尿病（採血で判明、発症時期はDM差分問診に従う）（受診理由の直後、空行なし）\n（内分泌の主病名は引き続き出力しない。医師が診察時に記載する）' : '（＃で始まる自院管理の主病名行は出力しない。医師が診察時に記載する）'}
...
（既往歴：要ドクター確認フラグありの場合のみ）□既往歴：要ドクター確認
□主病名：医師の診察時に確定・記載
${hasDm ? '□採血で DM判明 → DM初期評価追加実施済\n' : ''}${hasDm && (dm.eyeFundusCheck === '受けていない' || dm.eyeNotebook === '持っていない') ? '□糖尿病-眼科連携手帳をお渡し\n' : ''}${hasDm && dm.weightLoss === 'あり（3kg以上）' ? '□体重減少あり（3ヶ月以内に3kg以上）インスリン導入要検討\n' : ''}${needsCarePlan || hasDm ? '□初回療養計画書を作成済' : '（生活習慣病のチェックがないため「□初回療養計画書を作成済」の行は出力しない）'}
${staffFlagsBlock}（その他該当する申し送り事項があれば記載）
【診察にあたっての要望】（記載あれば内容を、なければ「なし」と記載）
---------------------------------------------
${hasDm ? COMMON_FOOTER : `${getCurrentMonth()}：
...
${hasDm ? buildDmDiffRules('＃糖尿病（採血で判明）を受診理由の直後（♯反応性低血糖疑いよりも前）に空行なしで記載し、必要なら発症時期も記載') + '\n' : ''}
【整形済みデータ】
飲酒歴：${alcoholText()}
喫煙歴：${smokingText()}
生活情報：${livingText()}
...
${voiceMemoBlock}${voicePastHistoryBlock}${voiceMemoNeedsReviewBlock}${needsDoctorReviewBlock}${dmDiffBlock}
【出力フォーマット】
${getCurrentMonth()}：${voiceMemoNote ? '（' + voiceMemoNote.replace(/^。/, '') + '）' : ''}${hasDm ? '（採血でDM判明したことにも触れる）' : ''}
${hasDm ? '＃糖尿病（採血で判明、発症時期はDM差分問診に従う）（受診理由の直後、空行なし）\n' : ''}♯反応性低血糖疑い
...
${staffFlagsBlock}（その他申し送り事項があれば記載）
【診察にあたっての要望】（記載あれば内容を、なければ「なし」と記載）
---------------------------------------------
...
`
  ...
  // ────── 睡眠時無呼吸症候群 ──────
    ...
    // dmDiff: 採血で DM 判明後にスタッフが追記したセクション（lib/dmDiff.js）
    const dm = d.dmDiff || {}
    const hasDm = hasDmDiff(d)
    const dmDiffBlock = buildDmDiffBlock(d.dmDiff)
...
- SAS症状チェックがある場合のみ【SASの症状】セクションを【FH】の前に挿入し、「・」区切りで横一列に記載${hasDm ?`
- 【DM差分情報】が下に提供されている: ＃糖尿病（採血で判明）を ＃SAS の直下（HT/HLよりも前）に追加し、必要なら発症時期も記載。【糖尿病の症状】セクションも【SASの症状】の下に追加（チェックがある場合のみ）。重要既往（胃癌・膵癌・IHD・脳梗塞）があれば♯行で記載。家族歴のDM/HLにも反映。
...
${voiceMemoBlock}${voicePastHistoryBlock}${voiceMemoNeedsReviewBlock}${needsDoctorReviewBlock}
【出力フォーマット】
${getCurrentMonth()}：（受診理由サマリー1〜2行。SAS区分（CPAP継続/検査希望）も含める${voiceMemoNote}${hasDm ?'。採血でDM判明したことも触れる' : ''}）
${sasMainName}（受診理由の直後、空行なし）${hasDm ?'\n＃糖尿病（採血で判明、発症時期はDM差分情報に従う）' : ''}
＃HT（該当時のみ、空行なし）
＃HL（該当時のみ、空行なし）
（上記【整形済みデータ】の「その他の病名・既往歴」が「なし」以外なら、1疾患1行で「♯病名（通院先）」の形式で必ず全て記載する。通院先が空なら「♯病名」のみ。整形済みデータの通院先表記をそのまま使い、JSONの hospital 値で上書きしない。空行なし）

${sasSymptomsText ? '【SASの症状】（チェックされた症状を「・」で横一列に記載。「その他」がある場合は末尾に「その他: ○○」を追加）\n' : ''}${hasDm && dm.dmSymptoms?.selected?.length ? '【糖尿病の症状】（チェックされた症状を「・」で横一列に記載）\n' : ''}【アレルギー歴】（なしまたは内容を同じ行に）
【FH】DM(-/+) HT(-/+) HL(-/+) APO(-/+) IHD(-/+)（FH DMの場合は誰かも記載）
【飲酒歴】（整形済みテキスト）
【喫煙歴】（整形済みテキスト）
【健診】
...
${footerTrailing}`
    ...
    throw new UnsupportedFormTypeError(form_type)
  }
