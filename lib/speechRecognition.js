...
export function useSpeechRecognition() {
  ...
  // 【エコー対策】Android Chrome/Edge は recognition.start() 自動再開時に
  // 前セッションの最後の確定テキストを新セッションで「エコー再認識」することがある。
  // 前セッション末の finals と再開時刻を覚えておき、再開直後の echo を strip する。
  const lastSessionFinalsRef = useRef('')   // 前セッションの確定テキスト全体
  const lastEndTimestampRef = useRef(0)     // onend 発火時の unixtime
  const ECHO_WINDOW_MS = 1500               // エコー判定の有効時間窓
  ...
  useEffect(() => {
    ...
    recognition.onresult = (event) => {
      ...
      // 【エコー対策】Android Chrome/Edge は再開直後に前セッションの finals を
      // 新セッションで再認識して echo してくることがある。
      // 再開から ECHO_WINDOW_MS 以内に、sessionFinals が前セッションの finals で
      // 始まっていたらその先頭部分を echo として strip する。
      const sinceEnd = Date.now() - lastEndTimestampRef.current
      const inEchoWindow =
        lastEndTimestampRef.current > 0 &&
        sinceEnd < ECHO_WINDOW_MS &&
        lastSessionFinalsRef.current.length > 0
      let cleanedFinals = sessionFinals
      if (inEchoWindow && cleanedFinals.startsWith(lastSessionFinalsRef.current)) {
        cleanedFinals = cleanedFinals.slice(lastSessionFinalsRef.current.length)
      }

      // 確定テキスト = 前セッション分 + 現セッションの finals(echo strip 済)
      const combined = (sessionBaseTextRef.current + cleanedFinals).trim()
    ...
    recognition.onend = () => {
      ...
      if (userIntentRecordingRef.current) {
        // 前セッションの finals(sessionBase からの delta)を echo 検出用に保存
        const previousSessionFinals = finalRef.current.slice(sessionBaseTextRef.current.length)
        lastSessionFinalsRef.current = previousSessionFinals
        lastEndTimestampRef.current = Date.now()
