...
import { buildVocabularyHint } from './voiceVocabulary';
//
// 設計:
...
function todayWareki() {
  ...
  return `R${reiwa}（令和${reiwa}年${now.getMonth() + 1}月${now.getDate()}日）`;
}

const AI_PROMPT = (diseases, ageContext, existingSummary) => `あなたは内科外来のベテラン医師です。医療事務スタッフが患者から聴取した既往歴リストを見て、診療上必要な追加質問を簡潔に提案してください。
...
【最重要・既知情報を絶対に再質問しない】
下記の「既存の既往歴サマリー」に書かれている情報には絶対に質問してはいけない。
判定手順:
  1. 各病名について、サマリーの括弧内を読む
  2. 病院名（「自治医大」「○○病院」「○○クリニック」「○○科」等）が書かれていれば、病院名は既知 → 質問禁止
  3. 治療内容（「手術」「全摘」「抗がん剤」「内服」「ステント」等）が書かれていれば、治療有無・内容は既知 → 質問禁止
  4. 時期（「H8」「R5」「平成○年」「令和○年」「○年前」「○年から」等）が書かれていれば、時期は既知 → 質問禁止
     ただし「○年前」と相対表現の場合は「正確な和暦は？」と聞いても OK（precision向上）
  5. 通院状況（「現在通院なし」「○○通院中」等）が書かれていれば、通院先は既知 → 質問禁止

例:
  「♯肺癌（5年前、自治医大で手術・抗がん剤治療、現在通院なし）」
  → 時期=判明（5年前は和暦化推奨だが既知）、病院=自治医大で判明、治療=手術・抗がん剤で判明、通院=なしで判明
  → 質問は0個（このdiseaseはsuggestionsから除外、または questions: []）

  「♯胆嚢炎（2024年、胆嚢全摘術施行、現在通院なし）」
  → 時期=2024年（西暦→和暦化推奨）、治療=全摘で判明、通院=なしで判明、病院=未記載
  → 質問1個: 「手術を実施した医療機関」のみ

【既存の既往歴サマリー（既に判明している情報）】
${existingSummary || '(なし)'}

【出力ルール】
- 必ず JSON 形式で出力（マークダウン記号 \`\`\`json は不要、JSON 本体のみ）
- 形式: { "suggestions": [ { "disease": "病名", "questions": ["質問1", "質問2"] }, ... ] }
- 各病名につき0〜3個の質問（既存情報で全て埋まっていれば 0 個=質問なしで OK、空配列を返す or その病名を suggestions から除外）
- 質問は事務スタッフが患者にそのまま聞ける口語で（専門用語の羅列は避ける）
- 質問の方針:
  - 手術系（〜癌、〜手術後、〜摘出 など）: 手術時期 + 実施した医療機関名（既に判明している情報は除く）
  - 通院中の慢性疾患: 通院中の医療機関名のみ（治療内容・薬の詳細は不要）
  - 精神疾患: 通院中の医療機関のみ（服薬詳細は聞かない、種類が膨大なため）
  - 眼科疾患: 術後 or 経過観察 + 通院中の眼科の医療機関名
- 時期の表現は和暦（H8、R5、平成8年、令和5年 など）で統一。西暦・○年前・年齢は使わない
- 既に明らかな情報（病名そのもの、サマリーに書いてある内容）は質問しない
...
- 回答内容を該当する病名の括弧内に追記（例: 「♯胆石」 → 「♯胆石（H28、上尾中央総合病院で胆嚢摘出）」）
- 時期の表現は和暦（H8、R5、平成8年、令和5年 など）で統一。西暦・○年前・年齢は絶対に使わない（既存サマリーに西暦や「○年前」があれば、今日の日付から計算して和暦に変換すること。例: 「5年前」→ 今日がR8なら R3）
 ...
 * @param {string} [existingSummary] - 既存の AI 整形済み既往歴サマリー（重複質問防止のためのコンテキスト）
 * @returns {Promise<{ ok: true, suggestions: Array<{ disease: string, questions: string[] }> } | { ok: false, error: string }>}
 */
export async function aiSuggestFollowups(diseases, context = {}, existingSummary = '') {
  if (!Array.isArray(diseases) || diseases.length === 0) {
    return { ok: false, error: '病名が入力されていません' };
  }
  const ageContext = context?.age ? `${context.age}歳` : '';
  const prompt = AI_PROMPT(diseases, ageContext, existingSummary);
