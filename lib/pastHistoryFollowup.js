...
【最重要・既知情報を絶対に再質問しない】
下記の「既存の既往歴サマリー」に既に書かれている情報については、絶対に質問しない。
例: 「♯胆嚢炎（2024年、胆嚢全摘術施行）」と既にあれば、「胆嚢炎の手術時期」「手術の有無」を質問してはいけない（既に判明済）。
代わりに、不足している情報のみ質問する（例: 上記なら「手術病院名」が未記載なのでそれを質問）。
...
- 時期の表現は和暦（H8、R5、平成8年、令和5年 など）で統一。西暦・○年前・年齢は絶対に使わない（既存サマリーに西暦があれば和暦に変換すること）
