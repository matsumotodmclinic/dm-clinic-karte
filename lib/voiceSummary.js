...
// system プロンプト（静的部分・1日単位で固定 → 1h TTL でキャッシュ）
// 役割宣言・今日の日付・語彙ヒント・ルールを含める。
// 患者個別データ（フォーム情報・発話）は user message に分離。
const buildCurrentIllnessSystem = () => {
...
const buildCurrentIllnessUser = (transcript, formData) => {
...
export async function summarizeForKarte(transcript, formData, formType) {
  ...
  const systemText = buildCurrentIllnessSystem()
  const userText = buildCurrentIllnessUser(transcript, formData)
...
// 既往歴整形用 system プロンプト（静的部分・1日単位で固定 → 1h TTL でキャッシュ）
const buildPastHistorySystem = () => {
  const today = todayWareki()
...
const buildPastHistoryUser = (transcript) => {
  return `【発話内容(音声認識テキスト、誤認識を含む可能性あり)】
${transcript}
...
export async function summarizeForPastHistory(transcript) {
  ...
  const systemText = buildPastHistorySystem()
  const userText = buildPastHistoryUser(transcript)
