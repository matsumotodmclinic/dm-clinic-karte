...
// ★配列であることを保証する版。undefined が来ても [] を返す。
//   dropOther() は入力が未定義だと文字列を返すので、`.join()` する側はこちらを使う
//   （2026-09-08: support セクションが丸ごと無いレコードで
//    `.join is not a function` になり、カルテが作れなくなった）
export function dropOtherList(v) {
  const list = Array.isArray(v) ? v : (v === null || v === undefined || v === '' ? [] : [v])
  return list.filter(x => x && x !== 'その他')
}
...
export function buildJobStr(d) {
  const src = d?.lifestyle || d?.history || {}
  const jobs = dropOtherList(src.job)
