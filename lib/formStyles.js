...
//
// ■ 色は「カテゴリ」だけを意味する
// 青=糖尿病関連 / 緑=甲状腺関連 / 紺=その他。フォームごとに色を変えない。
// 色トークンの正本は lib/uiTokens.js（勤怠アプリと同一値）。
//
// ■ 対象外
// components/{DmDiffEditor,DmDxNoteEditor,VoiceMemoSection}.js は
// 詳細画面用でサイズ体系が一回り小さく、互いにも微差があるためここには含めない。

import { UI } from './uiTokens.js'  // node から直接読む検証スクリプト用に拡張子を明示
 ...
 * 使い方: `const { inp, lbl, btn, sBox } = makeFormStyles(FORM_THEMES.dm)`
 */
export function makeFormStyles(theme) {
  const t = theme || FORM_THEMES.dm
