...
const GUIDES = [
  // ----- 糖尿病関連 -----
  { category: '🩺 糖尿病関連', items: [
    { id: 'dm',      label: 'DM 基本 (2 型糖尿病)', emoji: '🩺',  color: '#1a5fa8', ready: true,  href: '/help/dm',      desc: '6 ステップ完全攻略 + 音声入力 (現病歴/既往歴) + 受付コード運用 + 再生成' },
    { id: 't1d',     label: '1 型糖尿病 (成人)',     emoji: '💉',  color: '#c53030', ready: false, desc: 'CGM/CSII 機器、 インスリン療法、 低血糖対応 (準備中、 2026-06 公開予定)' },
    { id: 'gdm',     label: '妊娠糖尿病',            emoji: '🤰', color: '#c05c8a', ready: false, desc: 'GDM / 糖尿病合併妊娠、 産科連携 (準備中)' },
    { id: 'ped-t1d', label: '小児 1 型糖尿病',      emoji: '👶', color: '#3182ce', ready: false, desc: '小児・思春期、 親への説明、 保育園/学校連携 (準備中)' },
    { id: 'rh',      label: '反応性低血糖',          emoji: '⚡', color: '#b45309', ready: false, desc: 'RH、 食事との関連、 OGTT 判定 (準備中)' },
  ]},
  // ----- 甲状腺関連 -----
  { category: '🦋 甲状腺関連', items: [
    { id: 'thyroid', label: '甲状腺 6 種 (バセドウ/橋本/腫大/腺腫)', emoji: '🦋', color: '#0d7d6a', ready: false, desc: 'エコー所見ベースのフォーム選択、 確定診断の流れ (準備中)' },
  ]},
  // ----- その他 -----
  { category: '💤 その他', items: [
    { id: 'sas',     label: '睡眠時無呼吸症候群 (SAS)', emoji: '💤', color: '#5a4fb8', ready: false, desc: 'CPAP 継続 / 簡易 PSG 検査、 採血で DM 判明時の DM 差分問診 (準備中)' },
  ]},
]
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f5f7fa' }}>
      <div style={{ maxWidth: 900, margin: '0 auto', padding: 20 }}>
        ...
        <nav style={{ background: '#1a5fa8', color: '#fff', padding: '14px 18px', borderRadius: 10, marginBottom: 16 }}>
          <div style={{ fontSize: 19, fontWeight: 700 }}>📖 完全ガイド集</div>
                ...
                const body = (
                  <div style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                      ...
                      <div style={{ fontWeight: 700, color: it.ready ? it.color : '#888', fontSize: 14 }}>{it.label}</div>
                      {it.ready
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 10 }}>
            {OPS_TOPICS.map((it) => (
              <div key={it.id} style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                  <span style={{ fontSize: 22 }}>{it.emoji}</span>
                  <div style={{ fontWeight: 700, color: '#666', fontSize: 14 }}>{it.label}</div>
                  <span style={{ marginLeft: 'auto', fontSize: 10, background: '#f5f5f5', color: '#888', padding: '2px 8px', borderRadius: 10, fontWeight: 700 }}>準備中</span>
                </div>
                <div style={{ fontSize: 12, color: '#555', lineHeight: 1.6 }}>{it.desc}</div>
              </div>
        ...
        {/* ヘルプ Footer */}
        <div style={{ background: '#e8f5e9', borderRadius: 12, padding: 16, marginTop: 18, fontSize: 13, color: '#1b5e20', lineHeight: 1.7 }}>
          ...
          院長 or KartePlus 本部 (<a href="mailto:karteplus2026@gmail.com" style={{ color: '#1565c0' }}>karteplus2026@gmail.com</a>) までご相談ください。
        </div>
      </div>
    </div>
  )
