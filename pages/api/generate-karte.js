...
（患者フラグが「●患者疑い（出禁対象）」の場合）□●患者疑い（出禁対象・要確認）`
  ...
  // 既往歴の聞き漏れチェック未回答質問 (申し送り事項の【既往歴確認事項】に出力)
  const unansweredList = Array.isArray(d.voicePastHistory?.unansweredQuestions)
    ? d.voicePastHistory.unansweredQuestions.filter(u => u && u.disease && u.question)
    : []
  const unansweredBlock = unansweredList.length > 0
    ? `\n【既往歴の聞き漏れチェック・未回答質問(申し送り事項末尾に「【既往歴確認事項】」として必ず列挙、医師が患者に直接確認するため)】\n${unansweredList.map(u => `・${u.disease}：${u.question}`).join('\n')}\n`
    : ''

  // 既往歴聴取の時間切れフラグ (申し送り事項に医師確認依頼を追加)
  const needsDoctorReviewBlock = d.voicePastHistory?.needsDoctorReview
    ? `\n【既往歴聴取・時間切れフラグ(申し送り事項の冒頭近くに「□ 既往歴：聴取時間不足のため医師に確認依頼」を追加)】\nスタッフが時間切れで聴取しきれなかったため、医師に直接確認を依頼。\n`
    : ''
...
${voiceMemoBlock}${voicePastHistoryBlock}${unansweredBlock}${needsDoctorReviewBlock}
...
【事前聴取時　申し送り事項】
□通院のご案内をお渡し済
（既往歴聴取・時間切れフラグありの場合のみ）□既往歴：聴取時間不足のため医師に確認依頼
...
${STAFF_FLAGS}
（既往歴の聞き漏れチェック未回答質問がある場合のみ）【既往歴確認事項】（医師確認用、各項目を「・病名：質問内容」形式で箇条書き）
