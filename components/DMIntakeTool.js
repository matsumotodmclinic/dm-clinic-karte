...
export default function DMIntakeTool() {
  ...
  const buildLiving = () => {
    ...
    const spouse = livingSpouse || "";
    const other = (livingOther === "子供と同居なし") ? "" : livingOther || "";
    const custom = livingCustom || "";
    return [spouse, other, custom].filter(Boolean).join("・") || "";
  };
  ...
  const buildSendNote = () => {
    ...
    if (data.disease.thyroidAdded) notes.push("□健診・前医採血でLDL-C140mg/dl以上のため、甲状腺3項目を追加しました。");
...
甲状腺追加済：${data.disease.thyroidAdded}
...
（甲状腺追加済の場合）□健診・前医採血でLDL-C140mg/dl以上のため、甲状腺3項目を追加しました。
□生活習慣病療養計画書を作成済
