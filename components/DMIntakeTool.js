            ...
            <div key={i} style={{ display: "flex", gap: 8, marginBottom: 8, alignItems: "flex-start" }}>
              ...
              <div style={{ flex: 3 }}>
                ...
                {od.name ? (
                  <div>
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 3 }}>
                      {["自治医大さいたま医療センター","上尾中央総合病院","埼玉県立がんセンター","加藤泌尿器科","通院なし","その他"].map(h => <button key={h} style={{ ...btn(od.hospital === h), padding: "5px 10px", fontSize: 12 }} onClick={() => upOD(i, "hospital", h)}>{h}</button>)}
                    </div>
