    ...
    switch(step) {
      ...
      case 0: return (
        <div>
          ...
          <div style={{display:"flex",gap:8}}>
            {["あり","なし","不明"].map(v=>(
              <button key={v} style={btn(d.alert.weightLoss===v,v==="あり"?UI.danger.fg:undefined)}
      ...
      case 1: return (
        <div>
          ...
          <div style={{display:"flex",flexWrap:"wrap",marginBottom:14}}>
            ...
            <button style={btn(d.reason.dmConcern)} onClick={()=>{
            ...
            <div style={{paddingLeft:12,borderLeft:"3px solid #8e44ad",marginBottom:14}}>
              ...
              <div style={{display:"flex",flexWrap:"wrap",gap:3}}>
                {["家族に1型糖尿病の方がいる","急に体重が減った","喉が渇く・尿が多い","その他"].map(v=>(
                  <button key={v} style={btn(d.reason.dmConcernReason===v)} onClick={()=>up("reason","dmConcernReason",v)}>{v}</button>
                ))}
              </div>
            </div>
          )}
      ...
      case 2: return (
        <div>
          ...
          <div style={sBox({background:"#faf5ff",border:"1px solid #d6bcfa"})}>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:3,marginBottom:12}}>
              {["受給中","受給していない","不明"].map(v=>(
                <button key={v} style={btn(d.disease.pensionStatus===v)} onClick={()=>up("disease","pensionStatus",v)}>{v}</button>
              ))}
            </div>
              ...
              <div style={{paddingLeft:12,borderLeft:"3px solid #d6bcfa",marginTop:4}}>
                ...
                <div style={{display:"flex",flexWrap:"wrap",gap:3,marginBottom:12}}>
                  {["はい（加入していた）","いいえ（未加入）","不明"].map(v=>(
                    <button key={v} style={btn(d.disease.pensionKosei===v)} onClick={()=>up("disease","pensionKosei",v)}>{v}</button>
                  ))}
                </div>
              ...
              <div style={{marginBottom:10}}>
                ...
                <div style={{display:"flex",flexWrap:"wrap",gap:3,marginBottom:8}}>
                  {CHILD_LOCATIONS.map(v=>(
                    <button key={v} style={btn(d.history.childLocation===v)} onClick={()=>up("history","childLocation",v)}>{v}</button>
                  ))}
                </div>
                {d.history.childLocation&&d.history.childLocation!=="子供なし"&&(
                  <>
                    ...
                    <div style={{display:"flex",flexWrap:"wrap",gap:3}}>
                      {CHILD_GENDERS.map(v=>(
                        <button key={v} style={btn((d.history.childGender||[]).includes(v),undefined)} onClick={()=>toggleArr("history","childGender",v)}>{v}</button>
                      ))}
                    </div>
                  </>
                )}
      ...
      case 4: return (
        <div>
          ...
          {bmi&&(<div style={{marginBottom:16,padding:"10px 16px",background:"#fff5f5",borderRadius:8,fontSize:14,fontWeight:700,color:"#c53030"}}>BMI：{bmi}　{parseFloat(bmi)<18.5?"（低体重）":parseFloat(bmi)<25?"（普通体重）":parseFloat(bmi)<30?"（肥満1度）":"（肥満2度以上）"}</div>)}
          <label style={lbl()}>希望曜日（複数選択可）</label>
          <div style={{display:"flex",flexWrap:"wrap",gap:3,marginBottom:14}}>
            {WEEKDAYS.map(v=>(
              <button key={v} style={btn((d.body.preferredDays||[]).includes(v),UI.danger.fg)} onClick={()=>toggleArr("body","preferredDays",v)}>{v}</button>
            ))}
          </div>
          <label style={lbl()}>医師の希望</label>
          <div style={{display:"flex",flexWrap:"wrap",gap:3,marginBottom:14}}>
            {["指定なし","女性医師希望","男性医師希望","院長（初回のみ）"].map(v=>(
              <button key={v} style={btn(d.body.doctorGender===v,UI.danger.fg)} onClick={()=>up("body","doctorGender",v)}>{v}</button>
            ))}
          </div>
        ...
        {!done?(
          ...
          <div style={{background:UI.surface,borderRadius:8,padding:"22px 24px",border:`1px solid ${UI.border}`,boxShadow:"0 2px 8px rgba(0,0,0,.06)",border:"1px solid #c6f6d5"}}>
            ...
            <div style={{display:"flex",gap:8,marginTop:14,flexWrap:"wrap"}}>
              ...
              <button style={{flex:1,padding:"12px",borderRadius:8,border:"1px solid #c53030",background:"#fff5f5",color:"#c53030",fontWeight:700,fontSize:14,cursor:"pointer"}} onClick={()=>{setDone(false);setStep(0);setTimeout(scrollTop,50);}}>✏️ 修正する</button>
