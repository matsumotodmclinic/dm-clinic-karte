...
`;
    ...
    switch(step) {
      ...
      case 0: return (
        <div>
          ...
          <div style={{display:"flex",flexWrap:"wrap",gap:3,marginBottom:8}}>
            {KNOW_SOURCES.map(v=>(
              <button key={v} style={btn((d.reason.knowSource||[]).includes(v),"#0f9668")} onClick={()=>toggleArr("reason","knowSource",v)}>{v}</button>
            ))}
          </div>
      ...
      case 1: return (
        <div>
          ...
          <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:14}}>
            <button style={btn(d.reason.sasCategory==='cpap',"#1a5fa8",{padding:"10px 18px",fontSize:14})} onClick={()=>up("reason","sasCategory","cpap")}>CPAP治療の継続希望</button>
            <button style={btn(d.reason.sasCategory==='screening',"#0f9668",{padding:"10px 18px",fontSize:14})} onClick={()=>up("reason","sasCategory","screening")}>睡眠時無呼吸症候群の検査希望</button>
          </div>

          {d.reason.sasCategory==='cpap' && (
            <div style={sBox({background:"#fff7e6",border:"1.5px solid #f0c270"})}>
              <div style={{fontSize:13,fontWeight:800,color:"#a67000",marginBottom:8}}>📄 前医情報提供書（CPAP継続に必須）</div>
          ...
          {d.reason.sasCategory==='screening' && (
            <div style={sBox({background:"#f0fff4",border:"1.5px solid #9ae6b4"})}>
              <div style={{fontSize:13,fontWeight:800,color:"#276749",marginBottom:8}}>🔬 簡易PSG検査について</div>
          ...
          <div style={{display:"flex",flexWrap:"wrap",gap:3,marginBottom:8}}>
            {SAS_SYMPTOMS.map(sym=>{
              ...
              return (
                <button key={sym} style={btn(selected,"#6b3fa8",{padding:"6px 10px",fontSize:12})} onClick={()=>setData(p=>{
          ...
          <button style={{...btn(false,"#718096"),fontSize:13,marginBottom:14}} onClick={()=>setData(p=>{const a=[...(p.disease.otherDiseases||[]),{name:"",hospital:"",hospitalOther:"",dept:""}];return{...p,disease:{...p.disease,otherDiseases:a}};})}>＋ その他の病名を追加</button>
          ...
          <div style={{...sBox({background:"#f0f8ff",border:"1.5px solid #bee3f8"}),marginTop:8}}>
            ...
            <div style={{display:"flex",gap:16,flexWrap:"wrap"}}>
              ...
              <div style={{flex:1,minWidth:200}}>
                ...
                <div style={{display:"flex",flexWrap:"wrap",gap:3}}>
                  {["他院で施行済","健診で施行済","希望あり","希望なし"].map(v=>(<button key={v} style={{...btn(d.disease.echoAbdomen===v,"#2b6cb0"),padding:"6px 10px",fontSize:12}} onClick={()=>up("disease","echoAbdomen",v)}>{v}</button>))}
                </div>
              </div>
            </div>
          </div>
          ...
          {isOver70&&(<div style={sBox({border:"1.5px solid #fbd38d",background:"#fffaf0"})}>
            ...
            {(Array.isArray(d.history.livingOther)?d.history.livingOther:[]).includes("子供と同居なし")&&(
              <div style={{marginBottom:10}}>
                ...
                <div style={{display:"flex",flexWrap:"wrap",gap:3,marginBottom:8}}>
                  {CHILD_LOCATIONS.map(v=>(<button key={v} style={btn(d.history.childLocation===v,"#c05621")} onClick={()=>up("history","childLocation",v)}>{v}</button>))}
                </div>
                {d.history.childLocation&&d.history.childLocation!=="子供なし"&&(
                  <>
                    <label style={lbl({color:"#c05621",fontSize:11})}>息子・娘（複数選択可）</label>
                    <div style={{display:"flex",flexWrap:"wrap",gap:3}}>
                      {CHILD_GENDERS.map(v=>(<button key={v} style={btn((d.history.childGender||[]).includes(v),"#c05621")} onClick={()=>toggleArr("history","childGender",v)}>{v}</button>))}
                    </div>
                  </>
                )}
              </div>
            ...
            <div>
              ...
              <div style={{display:"flex",flexWrap:"wrap",gap:3,marginBottom:8}}>
                {["会社員（デスクワーク）","会社員（現場・営業）","自営業","パート・アルバイト","医療・福祉職","教育職（教師・保育士）","飲食・サービス業","農業・林業・漁業","専業主婦・主夫","学生"].map(v=>(
                  <button key={v} style={{...btn((d.history.job||[]).includes(v),"#1a5fa8"),padding:"6px 10px",fontSize:12}} onClick={()=>toggleArr("history","job",v)}>{v}</button>
                ))}
              </div>
      ...
      case 3: return (
        <div>
          ...
          <div style={{display:"flex",flexWrap:"wrap",gap:3,marginBottom:14}}>
            {["指定なし","女性医師希望","男性医師希望","院長（初回のみ）"].map(v=>(<button key={v} style={btn(d.body.doctorGender===v,"#2d8653")} onClick={()=>up("body","doctorGender",v)}>{v}</button>))}
          </div>
          ...
          <div style={sBox({background:"#fff8f0",border:"1.5px dashed #fbd38d",marginTop:14})}>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:3,marginBottom:10}}>
              {["通常","○患者疑い（話が長い方）","●患者疑い（出禁対象）"].map(v=>(<button key={v} style={btn(d.body.patientFlag===v,"#c05621",{fontSize:12})} onClick={()=>up("body","patientFlag",v)}>{v}</button>))}
            </div>
  ...
  return (
    <div ref={topRef} style={{minHeight:"100vh",background:"linear-gradient(135deg,#eef2fb 0%,#f5f4ff 60%,#fff5f0 100%)",fontFamily:"'Noto Sans JP','Hiragino Kaku Gothic ProN',sans-serif",padding:"20px 16px"}}>
      ...
      <div style={{maxWidth:680,margin:"0 auto 18px"}}>
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          <button onClick={()=>router.push("/")} style={{padding:"6px 10px",borderRadius:8,border:"1.5px solid #d6d8f0",background:"#fff",color:"#5a4fa8",fontWeight:700,fontSize:12,cursor:"pointer"}}>← トップ</button>
          <div style={{width:42,height:42,borderRadius:12,background:"linear-gradient(135deg,#5a4fa8,#8a7ed4)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:22}}>🌙</div>
          <div>
            <div style={{fontSize:11,color:"#7a6fd0",fontWeight:700,letterSpacing:"0.08em"}}>まつもと糖尿病クリニック</div>
            <div style={{fontSize:20,fontWeight:900,color:"#1a2a4a"}}>初診事前問診</div>
          </div>
          <div style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:8}}>
            <span style={{fontSize:12,background:"#eee9ff",color:"#5a4fa8",padding:"4px 14px",borderRadius:20,fontWeight:700}}>睡眠時無呼吸症候群</span>
          </div>
        </div>
      </div>
      ...
      <div style={{maxWidth:680,margin:"0 auto"}}>
        {!done&&(<div style={{display:"flex",gap:4,marginBottom:18}}>
          {STEPS.map((s,i)=>(<div key={s.id} onClick={()=>goStep(i)} style={{flex:1,textAlign:"center",cursor:"pointer",userSelect:"none"}}>
            <div style={{height:4,borderRadius:2,background:i===step?"#5a4fa8":i<step?"#8a7ed4":"#d6d8f0",marginBottom:4,transition:"background 0.3s"}}/>
            <div style={{fontSize:10,color:i===step?"#5a4fa8":i<step?"#8a7ed4":"#b0b8d8",fontWeight:i===step?800:i<step?600:400}}>{i<step?"✓ ":""}{s.title}</div>
          </div>))}
        </div>)}

        {!done?(
          <div style={{background:"#fff",borderRadius:16,padding:"24px 26px",boxShadow:"0 2px 20px rgba(90,79,168,0.07)"}}>
            <h2 style={{fontSize:16,fontWeight:800,color:"#1a2a4a",marginBottom:18,borderBottom:"2px solid #eee9ff",paddingBottom:10}}>{STEPS[step].title}</h2>
            {renderStep()}
            <div style={{display:"flex",justifyContent:"space-between",marginTop:26}}>
              <button style={{padding:"11px 22px",borderRadius:8,border:"1.5px solid #d6d8f0",background:"#f7f6ff",color:step===0?"#c0c4e0":"#5a4fa8",fontWeight:700,fontSize:14,cursor:step===0?"not-allowed":"pointer"}} onClick={()=>goStep(step-1)} disabled={step===0}>← 前へ</button>
              {step<STEPS.length-1?(
                <button style={{padding:"11px 26px",borderRadius:8,border:"none",background:"linear-gradient(135deg,#5a4fa8,#8a7ed4)",color:"#fff",fontWeight:800,fontSize:14,cursor:"pointer",boxShadow:"0 4px 12px rgba(90,79,168,0.25)"}} onClick={()=>goStep(step+1)}>次へ →</button>
              ):(
                <button style={{padding:"11px 26px",borderRadius:8,border:"none",background:loading?"#aab0d4":"linear-gradient(135deg,#0f9668,#34d399)",color:"#fff",fontWeight:800,fontSize:14,cursor:loading?"not-allowed":"pointer",boxShadow:"0 4px 12px rgba(15,150,104,0.25)"}} onClick={generateKarte} disabled={loading}>{loading?"生成中...":"✨ カルテ文を生成"}</button>
              )}
            </div>
          </div>
        ):(
          <div style={{background:"#fff",borderRadius:16,padding:"24px 26px",boxShadow:"0 2px 20px rgba(90,79,168,0.07)",border:"2px solid #d6d0f8"}}>
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}>
              <div style={{width:32,height:32,borderRadius:8,background:"#5a4fa8",display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontWeight:900,fontSize:16}}>✓</div>
              <div>
                <div style={{fontWeight:800,color:"#3a2f78",fontSize:15}}>カルテ記載文が生成されました</div>
            ...
            {visitCode && (
              <div style={{background:"linear-gradient(135deg,#5a4fa8,#8a7ed4)",borderRadius:14,padding:"20px",marginBottom:16,textAlign:"center"}}>
            ...
            <div style={{display:"flex",gap:8,marginTop:14,flexWrap:"wrap"}}>
              <button style={{flex:1,padding:"12px",borderRadius:8,border:"none",background:"linear-gradient(135deg,#5a4fa8,#8a7ed4)",color:"#fff",fontWeight:800,fontSize:14,cursor:"pointer"}} onClick={()=>copyToClipboard(result)}>📋 コピー</button>
