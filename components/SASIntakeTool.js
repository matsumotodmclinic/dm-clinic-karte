    ...
    switch(step) {
      ...
      case 1: return (
        <div>
          ...
          <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:14}}>
            <button style={btn(d.reason.sasCategory==='cpap',UI.primary.fg,{padding:"10px 18px",fontSize:14})} onClick={()=>up("reason","sasCategory","cpap")}>CPAP治療の継続希望</button>
            <button style={btn(d.reason.sasCategory==='screening',UI.success.fg,{padding:"10px 18px",fontSize:14})} onClick={()=>up("reason","sasCategory","screening")}>睡眠時無呼吸症候群の検査希望</button>
          </div>
          ...
          <div style={{display:"flex",flexWrap:"wrap",gap:3,marginBottom:8}}>
            {SAS_SYMPTOMS.map(sym=>{
              ...
              return (
                <button key={sym} style={btn(selected,UI.fixed.fg,{padding:"6px 10px",fontSize:12})} onClick={()=>setData(p=>{
          ...
          <div style={{...sBox({background:"#f0f8ff",border:"1.5px solid #bee3f8"}),marginTop:8}}>
            ...
            <div style={{display:"flex",gap:16,flexWrap:"wrap"}}>
              <div style={{flex:1,minWidth:200}}>
                ...
                <div style={{display:"flex",flexWrap:"wrap",gap:3}}>
                  {["他院で施行済","健診で施行済","希望なし"].map(v=>(<button key={v} style={{...btn(d.disease.echoNeck===v,UI.primary.fg),padding:"6px 10px",fontSize:12}} onClick={()=>up("disease","echoNeck",v)}>{v}</button>))}
                </div>
              </div>
              <div style={{flex:1,minWidth:200}}>
                <label style={lbl({color:"#2b6cb0"})}>腹部エコー</label>
                <div style={{display:"flex",flexWrap:"wrap",gap:3}}>
                  {["他院で施行済","健診で施行済","希望あり","希望なし"].map(v=>(<button key={v} style={{...btn(d.disease.echoAbdomen===v,UI.primary.fg),padding:"6px 10px",fontSize:12}} onClick={()=>up("disease","echoAbdomen",v)}>{v}</button>))}
                </div>
              </div>
            </div>
          </div>
          ...
          {isOver70&&(<div style={sBox({border:"1.5px solid #fbd38d",background:"#fffaf0"})}>
            ...
            {(Array.isArray(d.history.livingOther)?d.history.livingOther:[]).includes("子供と同居なし")&&(
              <div style={{marginBottom:10}}>
                ...
                <div style={{display:"flex",flexWrap:"wrap",gap:3,marginBottom:8}}>
                  {CHILD_LOCATIONS.map(v=>(<button key={v} style={btn(d.history.childLocation===v,UI.warning.fg)} onClick={()=>up("history","childLocation",v)}>{v}</button>))}
                </div>
      ...
      case 3: return (
        <div>
          ...
          <div style={{display:"flex",flexWrap:"wrap",gap:3,marginBottom:14}}>
            {["指定なし","女性医師希望","男性医師希望","院長（初回のみ）"].map(v=>(<button key={v} style={btn(d.body.doctorGender===v,UI.success.fg)} onClick={()=>up("body","doctorGender",v)}>{v}</button>))}
          </div>
          ...
          <div style={sBox({background:"#fff8f0",border:"1.5px dashed #fbd38d",marginTop:14})}>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:3,marginBottom:10}}>
              {["通常","○患者疑い（話が長い方）","●患者疑い（出禁対象）"].map(v=>(<button key={v} style={btn(d.body.patientFlag===v,UI.warning.fg,{fontSize:12})} onClick={()=>up("body","patientFlag",v)}>{v}</button>))}
            </div>
