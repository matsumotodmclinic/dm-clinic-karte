...
const initialData = {
  ...
  disease: { igt: false, ht: false, hl: false, echoNeck: "", echoAbdomen: "", otherDisease: "", otherDiseases: [{name:"",hospital:"",hospitalOther:""}] },
  history: {
...
export default function HTHLIntakeTool() {
  ...
  const handleSaveRetry = async () => {
    setSaveError(false);
    try {
      const saveRes = await fetch("/api/questionnaire",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({form_type:"高血圧・脂質異常症",form_data:data,age:data.history.age||null,generated_karte:result})});
...
`;
    try {
      ...
      try {
        const saveRes = await fetch("/api/questionnaire",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({form_type:"高血圧・脂質異常症",form_data:data,age:data.history.age||null,generated_karte:generated})});
        const saveJson = await saveRes.json();
        if(saveJson.visit_code) { setVisitCode(saveJson.visit_code); if (saveJson.id) setRecordId(saveJson.id); }
        else setSaveError(true);
