...
export default function EndocrineIntakeTool() {
  ...
  const generateKarte = async () => {
    ...
    try {
      ...
      const generated = await generateKarteText("内分泌", data, callGenerateApi);
      setResult(generated);
      ...
      case 0: return (
        <div>
          ...
          <div style={{display:"flex",flexWrap:"wrap",marginBottom:14}}>
            ...
            <button style={btn(d.reason.concern)} onClick={()=>{up("reason","concern",!d.reason.concern);if(d.reason.type)up("reason","type","");}}>
            ...
            <div style={{paddingLeft:12,borderLeft:"3px solid #8e44ad",marginBottom:14}}>
              ...
              <div style={{display:"flex",flexWrap:"wrap",gap:3}}>
                {["家族に内分泌疾患の方がいる","健診で異常を指摘された","頭痛・めまいがある","その他"].map(v=>(
                  <button key={v} style={btn(d.reason.concernType===v)} onClick={()=>up("reason","concernType",v)}>{v}</button>
                ))}
              </div>
            </div>
          )}
          ...
          <div style={sBox({background:"#fffaf0",border:"1px solid #fbd38d",marginBottom:14})}>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:3}}>
              {[["dm","糖尿病"],["ht","高血圧"],["hl","脂質異常症"]].map(([k,l])=>{
                ...
                return <button key={k} style={btn(on)} onClick={()=>upN("disease","carePlanDiseases",k,!on)}>{on?"✓ ":""}{l}</button>;
              })}
            </div>
          </div>
      ...
      case 2: return (
        <div>
          ...
          <div style={{paddingLeft:12,borderLeft:"3px solid #6b3fa8",marginBottom:14}}>
              ...
              <div key={i} style={{marginBottom:10}}>
                <div style={{display:"flex",flexWrap:"wrap",gap:3,marginBottom:5}}>
                  {["父","母","兄弟・姉妹","祖父","祖母","子"].map(v=>(
                    <button key={v} style={{...btn(f.who===v),padding:"5px 10px",fontSize:12}} onClick={()=>upFh(i,"who",f.who===v?"":v)}>{v}</button>
                  ))}
                </div>
            ...
            <button style={{...btn(false),fontSize:12}} onClick={addFh}>＋ 家族歴を追加</button>
          </div>
