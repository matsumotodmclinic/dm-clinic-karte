...
export default function EndocrineIntakeTool() {
  ...
  const handleSaveRetry = async () => {
    setSaveError(false);
    try {
      const saveRes = await fetch("/api/questionnaire",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({form_type:"内分泌",form_data:data,age:data.history.age||null,generated_karte:result})});
...
`;
    try {
      ...
      try {
        const saveRes = await fetch("/api/questionnaire",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({form_type:"内分泌",form_data:data,age:data.history.age||null,generated_karte:generated})});
        const saveJson = await saveRes.json();
        if(saveJson.visit_code) { setVisitCode(saveJson.visit_code); if (saveJson.id) setRecordId(saveJson.id); }
        else setSaveError(true);
      ...
      case 0: return (
        <div>
          ...
          <div style={{display:"flex",flexWrap:"wrap",marginBottom:14}}>
            ...
            <button style={btn(d.reason.concern,UI.fixed.fg)} onClick={()=>{up("reason","concern",!d.reason.concern);if(d.reason.type)up("reason","type","");}}>
          ...
          {d.reason.concern&&(
            <div style={{paddingLeft:12,borderLeft:"3px solid #8e44ad",marginBottom:14}}>
              ...
              <div style={{display:"flex",flexWrap:"wrap",gap:3}}>
                {["家族に内分泌疾患の方がいる","健診で異常を指摘された","頭痛・めまいがある","その他"].map(v=>(
                  <button key={v} style={btn(d.reason.concernType===v,UI.fixed.fg)} onClick={()=>up("reason","concernType",v)}>{v}</button>
                ))}
              </div>
            </div>
          )}
          {d.reason.type==="紹介"&&(<div style={sBox()}>
            ...
            <button style={{...btn(d.reason.referralQuickSelect,UI.success.fg),marginBottom:12,fontSize:14,padding:"10px 18px",border:d.reason.referralQuickSelect?"2px solid #0f9668":"2px dashed #0f9668",background:d.reason.referralQuickSelect?"#0f9668":"#f0fff8",color:d.reason.referralQuickSelect?"#fff":"#0f9668"}}
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:3}}>
              {["コントロール不良のため","安定していたため当院へ","専門的管理のため","転居のため","内容不明"].map(v=><button key={v} style={btn(d.reason.referralDetail===v)} onClick={()=>up("reason","referralDetail",v)}>{v}</button>)}
            </div>
          </div>)}
          ...
          <div style={sBox({background:"#fffaf0",border:"1.5px solid #fbd38d",marginBottom:14})}>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:3}}>
              {[["dm","糖尿病"],["ht","高血圧"],["hl","脂質異常症"]].map(([k,l])=>{
                ...
                return <button key={k} style={btn(on,UI.warning.fg)} onClick={()=>upN("disease","carePlanDiseases",k,!on)}>{on?"✓ ":""}{l}</button>;
              })}
            </div>
          </div>
                  ...
                  <div>
                    <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:4}}>
                      <select style={{...inp(),flex:2,fontSize:12}} value={od.hospital||""}
                        onChange={e=>setData(p=>{const a=[...(p.disease.otherDiseases||[])];a[i]={...a[i],hospital:e.target.value,dept:""};return{...p,disease:{...p.disease,otherDiseases:a}};})} >
                      ...
                      {od.hospital&&od.hospital!=="通院なし"&&od.hospital!=="加藤泌尿器科"&&od.hospital!=="その他"&&(
                        <select style={{...inp(),flex:1,fontSize:12}} value={od.dept||""}
                          onChange={e=>setData(p=>{const a=[...(p.disease.otherDiseases||[])];a[i]={...a[i],dept:e.target.value};return{...p,disease:{...p.disease,otherDiseases:a}};})} >
      ...
      case 2: return (
        <div>
          ...
          <div style={{paddingLeft:12,borderLeft:"3px solid #6b3fa8",marginBottom:14}}>
              ...
              <div key={i} style={{marginBottom:10}}>
                <div style={{display:"flex",flexWrap:"wrap",gap:3,marginBottom:5}}>
                  {["父","母","兄弟・姉妹","祖父","祖母","子"].map(v=>(
                    <button key={v} style={{...btn(f.who===v,UI.fixed.fg),padding:"5px 10px",fontSize:12}} onClick={()=>upFh(i,"who",f.who===v?"":v)}>{v}</button>
                  ))}
                </div>
            ...
            <button style={{...btn(false,UI.fixed.fg),fontSize:12}} onClick={addFh}>＋ 家族歴を追加</button>
          </div>
      ...
      case 3: return (
        <div>
          ...
          <div style={{display:"flex",flexWrap:"wrap",gap:3,marginBottom:14}}>
            {WEEKDAYS.map(v=>(
              <button key={v} style={btn((d.body.preferredDays||[]).includes(v),"#2d8653")} onClick={()=>toggleArr("body","preferredDays",v)}>{v}</button>
            ))}
          </div>
          <label style={lbl()}>医師の希望</label>
          <div style={{display:"flex",flexWrap:"wrap",gap:3,marginBottom:14}}>
            {["指定なし","女性医師希望","男性医師希望","院長（初回のみ）"].map(v=>(
              <button key={v} style={btn(d.body.doctorGender===v,UI.success.fg)} onClick={()=>up("body","doctorGender",v)}>{v}</button>
            ))}
          </div>
    ...
    <div ref={topRef} style={{minHeight:"100vh",background:"linear-gradient(135deg,#e8f4fd 0%,#f0f7ff 60%,#edf7ee 100%)",fontFamily:"'Noto Sans JP','Hiragino Kaku Gothic ProN',sans-serif",padding:"20px 16px"}}>
      ...
      <div style={{maxWidth:680,margin:"0 auto 18px"}}>
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          <button onClick={()=>router.push("/")} style={{padding:"6px 10px",borderRadius:8,border:"1.5px solid #c6eed0",background:"#fff",color:"#2d8653",fontWeight:700,fontSize:12,cursor:"pointer"}}>← トップ</button>
...
<span style={{fontSize:12,background:"#e8f8ee",color:"#2d8653",padding:"4px 14px",borderRadius:20,fontWeight:700}}>内分泌</span>
          </div>
        </div>
      ...
      <div style={{maxWidth:680,margin:"0 auto"}}>
        {!done&&(<div style={{display:"flex",gap:4,marginBottom:18}}>
          {STEPS.map((s,i)=>(<div key={s.id} onClick={()=>goStep(i)} style={{flex:1,textAlign:"center",cursor:"pointer",userSelect:"none"}}>
            <div style={{height:4,borderRadius:2,background:i===step?"#2d8653":i<step?"#48bb78":"#d0dff5",marginBottom:4,transition:"background 0.3s"}}/>
            <div style={{fontSize:10,color:i===step?"#2d8653":i<step?"#48bb78":"#b0c8e0",fontWeight:i===step?800:i<step?600:400}}>{i<step?"✓ ":""}{s.title}</div>
          </div>))}
        </div>)}
