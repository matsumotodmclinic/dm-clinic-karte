...
├── components/
│   ├── FormHeader.js              # 全フォーム共通ヘッダー（携帯幅は 2 段・iPad/PC は 1 行）
...
## アーキテクチャ: カルテ生成（2026-09-07〜 テンプレート＋統合）

★**カルテ本文の組み立ては `lib/buildKarteTemplate.js` の 1 箇所だけ。全 14 フォーム。**
AI に全文を書かせるのはやめた。初回生成も再生成も `lib/generateKarte.js` を通るので、
「初回生成と再生成でカルテが違う」事故は構造的に起きない。

```
generateKarteText(form_type, form_data, callAI)
  ├ buildMergePrompt()  → 文章化が要る自由記述が無ければ null（= AI を呼ばない）
  ├ callAI()            → 統合の2点だけを頼む（全文生成の約1/10）
  └ buildKarteTemplate(form_type, form_data, { merged })   ← 常にここが本文を組む
```

- AI を呼ぶ条件は **「文章化が要る自由記述があるとき」**＝ ①音声入力 ②キーボードの自由記入欄
  （受診理由の自由記入・「気になる理由」の詳細・SASのその他・甲状腺の補足）。
  **選択式のボタンだけで完結した回は AI を呼ばない**（決定論・即時・無料）
- そのとき AI がやるのは **①話し言葉の文章化と構造化データの統合 ②♯既往のマージ** の 2 つだけ
  （和暦への換算もここ。素通しだと「3年前」がそのままカルテに載る）
- ★**音声そのものの整形（校正）は入力時に `lib/voiceSummary.js` が従来どおり実施**。ここは変えていない
- AI が落ちても壊れた返答でも **JS 素組みのカルテを必ず返す**（生成できない状態を作らない）
- **甲状腺6フォームは音声入力を持たないので常に AI ゼロ**

### 経路A: フォーム送信時（メイン）
`components/*IntakeTool.js` の `generateKarte()` → `generateKarteText()` →（必要なら）
`/api/generate` プロキシ → `/api/questionnaire` POST で保存

### 経路B: 詳細画面で再生成
`detail/[id].js` → `/api/generate-karte` → 同じ `generateKarteText()`
→ `/api/questionnaire` PATCH で上書き

違いは **dmDiff（採血で糖尿病が判明したときの追加聴取）だけ**。これは詳細画面でしか入力できないので
経路A では常に無い状態で同じ関数に入る。

### 逃げ道: 旧方式（AI が全文を書く）
詳細画面の「🅰 旧AI版で作り直す」= `POST /api/generate-karte?legacy=1` が
`lib/buildKartePrompt.js`（1,097行）で従来どおり AI に全文を書かせる。
テンプレート版の出力がおかしい患者が出たとき、デプロイを待たずにその場で戻すためのもの。
**通常運用では使わない。**
✅ **2026-09-24: 院長が外来で実患者のカルテを確認し「特に問題なし」**（切替 09-07 から 17 日・指摘ゼロ）。
**消す条件は満たされた。あとは実行するだけ**（院長の GO 待ち）。消す範囲 = 詳細画面の
「🅰 旧AI版で作り直す」ボタン / `?legacy=1` 分岐 / `lib/buildKartePrompt.js` /
ゴールデンの**プロンプト全文スナップショット層**（旧方式を守るためだけの層）。
**カルテ本文の出力は 1 文字も変わらない**（通常経路は `buildKarteTemplate.js` のみ）。

## バグ防止機構（2026-08-22 導入）

```bash
npm run check          # スタイル検査 + ゴールデンテスト + eval（コミット前に必ず）
npm test               # ゴールデンテスト（268件）だけ
npm run eval           # カルテ生成 eval（14フォーム×5例＝70ケース・API 不要）
npm run eval -- --show DM基本/難渋   # そのケースのカルテ本文と統合材料を全文表示
npm run eval -- --ai   # 自由記述のあるケースだけ実際に Claude を呼ぶ（要 API キー・3回で揺れも見る）
...
### ① カルテ組立の単一実装（2026-09-06〜）
`scripts/check-prompt-sync.mjs` と `scripts/prompt-sync-allow.json` は**役目を終えて削除**した。
比較する相手（コンポーネント側のプロンプト）が無くなったため。

代わりに `prompt.test.mjs` に **二重管理を復活させないテスト 3 本**を置いている:
- `components/*IntakeTool.js` に `const prompt = \`` が現れたら落ちる
- 全 IntakeTool が `generateKarteText()` を呼んでいなければ落ちる
- コンポーネントが `/api/generate` を直接叩いていたら落ちる

**カルテを直すときは `lib/buildKarteTemplate.js` の 1 箇所だけ**。9 ファイル同期はもう不要。
...
- `prompt.test.mjs`: 4層
  ...
  2. カルテ本文のアサーション（「鰐坂医院を含む」「子宮筋腫（その他）を含まない」等）
  3. **カルテ本文の全文スナップショット**（`__snapshots__/カルテ_*.txt`）← ★これが本番の出力そのもの
  4. プロンプト全文スナップショット（`__snapshots__/*.txt`）← 旧方式（`?legacy=1`）を守るためのもの
- `coverage.test.mjs`: **取りこぼし検査**（下記③）
- 和暦の現在月（R8.8 等）は `R{NOW}` に伏せてから比較する（実行日で変わるため）

**検証済み**: 修正前のバグ実装に戻すと 20件が落ちる（= 2026-08-22 の2バグを実際に捕捉できる）。

★**フィクスチャは必ず `initialData` から起こすこと。** 実データと形が違うと
「テストは通るのに本番で落ちる」。2026-09-06・09-07 の 2 回とも、これが原因の
取りこぼしが見つかっている（SAS の SAS区分が丸ごと「未選択」になっていた 等）。

### ③ 取りこぼし検査（`scripts/golden/coverage.test.mjs`、全14フォーム）
form_data の全フィールドを 1 つずつ書き換えて、**カルテ本文か統合プロンプトのどちらかが変わるか**を見る。
どちらも変わらなければ、その項目はカルテに一切影響していない = **情報が黙って落ちている**。

- **自由記述なしの状態で検査する**（あると統合プロンプトが何でも運んでしまい検査が緩くなる）
- 例外は `COMMON_ALLOWED` / `ALLOWED[フォーム]` に**理由付きで**登録する。理由が書けないものは直す
- 逆に「理由リストに載っているのに実は使われている」も検出する（棚卸し）
- 🔴 **新しいフォームを足すときは必ずここにも足すこと**

★ これがあるので「質を落とさない」が**約束でなく検査**になる。

### ④ カルテ生成 eval（`scripts/eval/`、2026-09-08 新設）
情報提供書アプリ（voice）の出力 eval と同じ形。違いは **問診の本文は JS が決定論的に組むので
API 無しで検証できる**こと（AI が関わるのは自由記述の文章化だけなので、そこだけ `--ai` で見る）。

- `cases.mjs`: **14 フォーム × 5 例 = 70 ケース**。voice の 59 ケースに倣い
  「長さでなく**入力の壊れ方**」で攻める = 最小 / 標準 / 複雑 / **難渋** / 分岐
  （難渋 = 「その他」多用・話し言葉の自由記入・矛盾入力・空欄だらけ・記号混入）
- `rules.mjs`: 機械判定。error（破損・情報の創作・必須欄の欠落）/ warn（書式）/ info（気付き）
- `eval-rules.test.mjs`: ★**ルール自体が赤になることの確認**（壊したカルテを食わせる 25 件）。
  これが無いと「eval が全緑」に意味が無い

> ★★ **機械判定だけでは 1 件も出なかった。難渋症例の本文を人が読んで 8 件見つかった**（2026-09-08）。
> voice の教訓「検証は 機械の走査（CI）＋ 実データでの突き合わせ（人）の 2 段構え」がそのまま当たった。
> **新しいフォームやフィールドを足したら、eval にケースを足して 難渋 の本文を一度は目で読むこと。**
...
- **プロンプトを直す場所は `lib/buildKartePrompt.js` だけ**
  （2026-09-07 以降、そのプロンプト自体が旧方式 `?legacy=1` 専用になった。
  　通常のカルテを直す場所は `lib/buildKarteTemplate.js`）
...
- viewport を入れたので携帯の問診フォームは実寸で描画される。幅 375px ではフォーム上部のヘッダーが
  折り返して崩れたため、同日に **`components/FormHeader.js` に集約して携帯幅 (600px 以下) は 2 段
  （1 段目: ← トップ / 完全ガイド / フォーム名チップ、2 段目: 施設名 + タイトル）** にした。
  9 フォーム (+甲状腺) の直書きヘッダーはこの部品の呼び出し 1 行に置き換え済み。iPad・PC は従来どおり 1 行。
  inline style では @media が書けないので、この部品だけ `<style>` を持つ（CSS ファイルは作らない方針のまま）。
  甲状腺のヘッダーだけ旧デザインのミント色（#a7f3d0 / #e6fff8 / #0d7d6a）が残っていたのを UI.success に揃えた
...
→ DM基本だけで試作していたテンプレート版を 14 フォーム全てに広げ、本番の生成経路にした。

#### 何が変わったか

| | 旧（〜2026-09-06） | 新 |
|---|---|---|
| カルテ本文 | AI が全文を書く | `lib/buildKarteTemplate.js` が組む |
| AI の役割 | 全部 | **音声があるときだけ**「受診理由サマリーの統合」「♯既往のマージ」 |
| 音声なしの初診 | AI 1回（6,363字→906字） | **AI ゼロ**（決定論・即時・無料） |
| 甲状腺6フォーム | AI 1回 | **常に AI ゼロ**（音声入力を持たないフォーム） |
| AI が落ちたとき | 生成失敗 | JS 素組みのカルテを必ず返す |

#### 空行の扱いを統一した
旧プロンプトの「出力フォーマット」の雛形には、同じプロンプト内の【空行ルール5条件】と
**矛盾する空行**が残っていた（例: 1型・高血圧は ♯他院管理 と【アレルギー歴】の間に空行がある雛形なのに、
ルールは「空行なし」と書いてある）。AI がどちらを採るかは毎回変わっていた。
テンプレート版では**常にルール側**を採る。フォーム間で並びが揃った。

#### 展開の過程で見つかった実バグ 1 件
問診票の体重減少ボタンは **「あり / なし / 不明」**（画面で「3ヶ月以内に3kg以上」と定義している）。
`"あり（3kg以上）"` という値は **DM差分問診にしか存在しない**。
2026-09-06 のテンプレート試作は後者だけを見ていたため、**DM基本・1型で
体重減少の警告と申し送りが一度も出ない**状態だった。`hasWeightLoss()` で両方を受ける。

⇒ 教訓: **列挙値は UI の選択肢そのものを見て書く。** 似た文字列を推測で書かない。

#### フィクスチャが実データと違っていた（2 回目）
- **SAS**: `sasCategory` / `cpapPriorRecordsConfirmed` / `sasSymptoms` を `disease` 配下に置いていた
  （実際は `reason` / `symptom` 配下）。プロンプトの SAS区分がずっと「未選択」のまま通っていた
...
2. **DM差分問診の「過去のHbA1c指摘」「治療の希望」「自由記載」を申し送りに出すようにした**。
   旧方式では AI に渡してはいたが、使うかどうかは毎回変わっていた
...
#### 逃げ道を残した
詳細画面の「🅰 旧AI版で作り直す」= `POST /api/generate-karte?legacy=1`。
`lib/buildKartePrompt.js` は消さずに残してある。
**数週間、実患者のカルテを見て問題が出なければボタンごと消してよい。**

#### 未解決（院長判断待ち）
- **甲状腺フォームは 20歳時体重・最大体重 を聴取しているのにカルテに出していない**
  （旧プロンプトから同じ。`coverage.test.mjs` の allow-list に ⚠️ 付きで記録）
...
この検査を CI に載せてテスト **134 → 151 件**（`npm test` の現在値）。
